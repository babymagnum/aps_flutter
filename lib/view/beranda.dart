...
class BerandaState extends State<Beranda> {
  ...
  onItemListMenuTap(Menu menu, int position, BuildContext context) {
    if (menu.action != "") {
      ...
      switch (menu.id) {
        ...
        //daftar karyawan
        case 10:
          Navigator.push(context, MaterialPageRoute(builder: (context) => DaftarKaryawan()));
          break;
  ...
  Widget wMenuTop({image, title, description, backgroundColor}) => Expanded(
    child: Container(
      decoration: BoxDecoration(
        borderRadius: BorderRadius.all(Radius.circular(5)),
